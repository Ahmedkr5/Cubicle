...
export default function OutlinedCard(props) {
  ...
  const photo = userProf?.map((prof) => prof.profileimage);
  const [open, setOpen] = useState('none');
  const userName = userProf?.map(
    ...
    await setMsgs(
      messages?.filter(
        (msg) =>
          ...
          (msg.transmitter === props.userck &&
            msg.receiver === transmitter &&
            msg.file)
      )
    );
  ...
  return (
    <>
      ...
      <div
        style={{ paddingTop: '75px', maxHeight: '880px', display: 'scroll' }}
      >
                ...
                <Fab size='small'>
                  <IconButton aria-label='delete'>
                    <Badge badgeContent={17} color='primary'>
                      {' '}
                      <MonetizationOnIcon fontSize='30' />{' '}
                    </Badge>
                  </IconButton>
                </Fab>
              </Typography>
