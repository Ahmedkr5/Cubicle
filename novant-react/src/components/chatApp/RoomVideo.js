...
export default function RoomVideo() {
        ...
        Swal.queue([{
          ...
          preConfirm: () => {
           var x = document.getElementById('myaudiocall');
            ...
            modalWindow = window.open("/meet/" + msg.data[0].year, modalTitle, "width=1200,height=900,menubar=no,resizable=no,scrollbars=no,status=no,location=no , top=500, left=500");
          }
  ...
  const handleClickOpen = () => {
    setTop100Films(users?.map(user=>({title:user.firstname+' '+user.lastname,
    year: user._id})).filter(u=>u!=transmitter))
    setOpen(true);
  };
  ...
  var modalWindow;
  const modalTitle = "Video Call";
  modalWindow = window.open("/meet/" + transmitter, "width=1200,height=900,menubar=no,resizable=no,scrollbars=no,status=no,location=no , top=500, left=500");
  ...
  return (
    <div className={classes.root}>
      ...
      onChange={(event, newValue) => {
        setSelectedUsers([...top100Films?.filter(u=>u.year==transmitter),...newValue]) ;
      }}
      ...
      style={{ width: 500 }}
      renderInput={(params) => (
        <TextField {...params} label="Add Users" variant="outlined" placeholder="Add Users" />
      )}
    />
          </DialogContentText>
        </DialogContent>
