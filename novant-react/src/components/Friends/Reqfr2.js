...
export default class Reqfr2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      users: [],
  ...
  componentDidMount() {
    ...
    reqs.then((requests) => {
      this.setState({ requests: requests }, () => {
        console.log('statereq:', this.state.requests.friendRequests);
      });
    });
  }
  render() {
    ...
    return (
      <>
        {users
          ?.filter((m) => requests?.friendRequests?.includes(m._id))
          .map((msg) => (
            <Paper
              elevation={0}
              style={{
                  ...
                  <>
                    ...
                    <ListItem style={{ display: 'flex', flexDirection: 'row' }}>
                      <Button
                        ...
                        onClick={() => {
                          this.setState((state) => {
                            const list = state.requests?.friendRequests?.filter(
                              (i) => i !== msg._id
                            );
                            axios
                              .put(
                                'https://mycubicle.herokuapp.com/users/frreq/' +
                                  currentuser['id'],
                                {
                                  friendRequests: list,
                                }
                              ...
                              .then(() => {
                                this.setState((state) => {
                                  const friend = [
                                    ...state.requests?.friends,
                                    msg._id,
                                  ];

                                  axios
                                    .put(
                                      'https://mycubicle.herokuapp.com/users/fr/' +
                                        currentuser['id'],
                                      {
                                        friends: friend,
                                      }
