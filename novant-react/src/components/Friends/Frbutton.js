...
const userid=currentuser['id'];
  ...
  return (
    <>
    {(this.props?.id !== currentuser['id'] && this.props?.myfriends?.find(m => m === this.props?.id  )) &&
...
onClick={()=>{ this.setState(state => {
  const nofr = this.props?.friends?.filter(i => i !== currentuser['id']);
  const nofr2 = this.props?.myfriends?.filter(i => i !== this.props?.id);
 axios.put("http://localhost:3001/users/fr/" + this.props?.id, {
friends:nofr,
...
remove friend</Button> } 
      ...
      {
      (this.props?.myfriends?.find(m => m === this.props?.id)  || this.props?.id === currentuser['id'] )   ?
      ...
      <Button  
       ...
       onClick={() => {
        ...
        this.setState(state => {
          ...
          axios.put("http://localhost:3001/users/frreq/" + this.props?.id, {
